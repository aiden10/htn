...
class ShutterdexRuntime:
    ...
    # A full Habitat frame can contain many sprite commands. Two seconds keeps
    # even a busy scene below the HTN service's 20-command/sec badge limit.
        ...
        *,
        ...
        async with self._lock_for(htn_id):
            ...
            if (
                ...
                # A Habitat advance is a server transaction, not UI state.
                # Run it before building the next frame so the event and new
                # creature positions appear from the same button press.
                await self._on_habitat_advance(badge.player_id)
                context = self._context_for_badge(badge)
                stored = self._ensure_session(badge, context)
                session = self._session_state(stored, context)
    ...
    def _watch_delivery(self, htn_id: str, tickets: Iterable[CommandTicket]) -> None:
